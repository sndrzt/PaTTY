...
static void clipme2(Terminal *term, pos top, pos bottom, bool rect, bool desel,
                   const int *clipboards, int n_clipboards, char *lastline)
{
    clip_workbuf buf;
    int old_top_x;
    int attr;
    ...
    /* Finally, transfer all that to the clipboard(s). */
    {
        ...
        for (i = 0; i < n_clipboards; i++) {
            if (clipboards[i] == CLIP_LOCAL) {
                ...
                sprintf(lastline, "%S", buf.textbuf);
            }
        }
        if (clip_local) {
            sfree(term->last_selected_text);
...
void term_copylastline(Terminal *term, const int *clipboards, int n_clipboards, char *lastline)
{
    pos top;
    pos bottom;
    tree234 *screen = term->screen;
    top.y = find_last_nonempty_line(term, screen);
    top.x = 0;
    bottom.y = find_last_nonempty_line(term, screen);
    bottom.x = term->cols;
    clipme2(term, top, bottom, false, true, clipboards, n_clipboards, lastline);
}
